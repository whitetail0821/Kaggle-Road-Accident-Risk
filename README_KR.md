...
* **변수 처리:** 'road_type', 'lighting', 'time_of_day' 등 범주형 특징에 대해 One-Hot Encoding을 적용하여 모델 학습 준비.
* **이상치 처리:** 사고 건수(num_reported_accidents)의 왜곡(Skewness)을 완화하기 위해 **로그 변환**을 시도했으나, Public Score에 유의미한 개선이 없어 최종 모델에서는 제외하고, 대신 하이퍼파라미터 튜닝에 집중했습니다.

### B. 베이스라인 모델 구축 및 분석

* **모델 선택:** 회귀 문제에서 뛰어난 성능을 보이는 **LightGBM**을 베이스라인 모델로 선정.
* **초기 성과:** 초기 튜닝 (learning_rate=0.05, n_estimators=1000) 후 Validation RMSE 0.0559 최종 달성.
* **특징 중요도 분석:** 'curvature' 와 'speed_limit' 이 사고 위험도 예측에 가장 중요한 특징임을 확인하고, 이 특징들을 중심으로 튜닝 및 개선 방향을 설정함.
...
* **튜닝의 한계 인지:** 파라미터 튜닝만으로는  RMSE 0.05566의 성능 벽을 깨기 어렵다는 것을 확인.
* **미래 전략:** 다음 프로젝트에서는 성능을 극한으로 끌어올리기 위해 LGBM과 XGBoost 예측값을 섞는 앙상블 기법** 및 **심층적인 특징 공학**에 집중하여 RMSE를 더욱 낮출 계획임.
